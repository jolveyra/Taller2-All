...
contract Katas {
     ...
     */
    function countBy(uint256 _x, uint256 _n)
        external
        pure
        returns (uint256[] memory _result)
    {
        require(_x >= 0 && _n >= 0, "Invalid input");

        _result = new uint256[](_n);
     ...
     */
    function isFactor(uint256 _base, uint256 _factor)
        public
        pure
        returns (bool)
    {
     ...
     */
    function centuryOf(uint256 _year)
        external
        view
        returns (uint256 _century)
    {
        uint currentYear = block.timestamp / 31556926 + 1970; // Get the current year using UNIX timestamp
        require(_year > 0 && _year <= currentYear, "Invalid _year");
     ...
     */
    function repeatMe(uint256 _n, string memory _s)
        external
        pure
        returns (string memory _result)
    {
        require(bytes(_s).length > 0, "Invalid string");

        // Initialize an empty string to store the result
        _result = "";

        // Repeat the string _s exactly _n times and concatenate
        for (uint256 i = 0; i < _n; i++) {
            _result = string(abi.encodePacked(_result, _s));
        }
    }
     ...
     */
    function trianglesGeometry(uint256 _a, uint256 _b)
        external
        pure
    ...
    function maxCombination(
        ...
        uint256 _b,
        uint256 _c
    ) external pure returns (uint256 _result) {}
}
