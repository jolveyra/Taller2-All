...
contract Rock_Paper_Scissor {
    ...
    function joinGame(uint256 _gameNumber) external payable {
        ...
        if (msg.value > game[_gameNumber].apuesta) {
            payable(msg.sender).transfer(msg.value - game[_gameNumber].apuesta);
        }

        require(msg.sender == game[_gameNumber].jugador2, "Not authorized");

        require(game[_gameNumber].estado == Status.created, "Game contenders are full");

        game[_gameNumber].estado = Status.open;
        
        emit GameStarted([game[_gameNumber].jugador1, game[_gameNumber].jugador2], _gameNumber);
    }
    ...
    function makeMove(uint256 _gameNumber, bytes32 _movement) external {
        require(game[_gameNumber].estado == Status.open, "Game not open");
        if(msg.sender == game[_gameNumber].jugador1){
            require(game[_gameNumber].movimientoJugador1 == Movement.none, "Your move is already set");
            game[_gameNumber].movimientoJugador1 = Movement(uint8(uint256(_movement)));
        } else {
            require(game[_gameNumber].movimientoJugador2 == Movement.none, "Your move is already set");
            game[_gameNumber].movimientoJugador2 = Movement(uint8(uint256(_movement)));
        }
    }
